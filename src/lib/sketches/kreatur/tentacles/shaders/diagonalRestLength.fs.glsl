...
//    vec3 tlPos = texture2D(tPosition, vUv + vec2(-uTexelSize.x, uTexelSize.y)).xyz;
//    vec3 trPos = texture2D(tPosition, vUv + vec2(uTexelSize.x, uTexelSize.y)).xyz;
//    vec3 blPos = texture2D(tPosition, vUv + vec2(-uTexelSize.x, -uTexelSize.y)).xyz;
//    vec3 brPos = texture2D(tPosition, vUv + vec2(uTexelSize.x, -uTexelSize.y)).xyz;
//
...
//    }
    ...
    data = vec4(pos.xyz, length(pos.xyz));
